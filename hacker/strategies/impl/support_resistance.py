...
class SupportResistance(Strategy):
    ...
    def analyze(self, analysis: MarketAnalysis, candles: list[Candle]) -> StrategyResult:
        ...
        if nearest_support is not None and (close - nearest_support) <= 0.4 * atr:
            if candles and candles[-1].is_bullish:
        ...
        if nearest_resistance is not None and (nearest_resistance - close) <= 0.4 * atr:
            if candles and candles[-1].is_bearish:
                return StrategyResult(
