...
def test_target_candle_start_aligns_to_boundary():
    ...
    when = datetime(2026, 8, 14, 12, 0, 30, tzinfo=timezone.utc)
    assert eng.target_candle_start(Timeframe.M1, when) == datetime(
        2026, 8, 14, 12, 1, 0, tzinfo=timezone.utc
    )


def test_send_at_uses_lead_time():
    eng = SignalTimingEngine(lead_seconds=17.0)
    target = datetime(2026, 8, 14, 12, 1, 0, tzinfo=timezone.utc)
...
def test_stale_rejection():
    eng = SignalTimingEngine(lead_seconds=17.0)
    target = datetime(2026, 8, 14, 12, 1, 0, tzinfo=timezone.utc)
    assert eng.is_stale(target, datetime(2026, 8, 14, 12, 0, 50, tzinfo=timezone.utc))
    assert not eng.is_stale(target, datetime(2026, 8, 14, 12, 0, 30, tzinfo=timezone.utc))
